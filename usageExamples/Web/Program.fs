...
module Views =
              ...
              [ (match username with
                 | Some _ ->
                     div []
                         [ a
                             [ _href "javascript:void(0);"
                               _id "openModal" ] [ encodedText "Click here" ]
    ...
    let signIn (model: Message) =
        [ h1 [ _id "header" ] [ encodedText model.Header ]
          div
              [ _id "ErrorMessage"
                _style "display:none;" ] [ encodedText "Form invalid" ]
          form
...
let configureApp (app: IApplicationBuilder) =
    ...
    (match env.EnvironmentName = "development" with
     ...
     | false -> app.UseGiraffeErrorHandler errorHandler).UseHttpsRedirection().UseCors(configureCors).UseStaticFiles()
        .UseGiraffe(webApp)
...
let main _ =
    ...
    WebHostBuilder().UseKestrel().UseContentRoot(contentRoot).UseWebRoot(webRoot)
